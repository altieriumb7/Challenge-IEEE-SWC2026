...
class FairCityEnv(gym.Env):
    def __init__(self, data_path="smart_city_traffic_mobility.csv"):
        ...
        print("Caricamento Dataset Completo (204.000 righe)...")
        self.full_df = pd.read_csv(data_path)
        self.timestamps = self.full_df['timestamp'].unique()
        
        # Build Adjacency Graph once based on the 100 unique coords
        ...
        self.max_steps = 1  # Episodic sampling: 1 azione = 1 ora di simulazione
        self.global_emission_cap = 500.0 # Soglia Sanitaria di Legge
    ...
    def reset(self, seed=None, options=None):
        ...
        # EPISODIC SAMPLING: Pesca un'ora storica casuale dal dataset!
    ...
    def step(self, action):
        ...
        for _ in range(3):
            ...
            for i in range(len(agg_df)):
                ...
                # Health Shield: 500 kg Global Emission Cap (Panic Routing se l'AI fallisce)
        ...
        # Funzione di Ricompensa (L'AI deve imparare a minimizzare le code senza far scattare il cap)
        r_env = - (max_vuln_emission * 2.0)
        r_spa = - (wait_var * 1.5)
        r_mod = - (avg_bus_wait * 3.0)
        r_glo = - (global_avg_wait * 1.0)
        
        # Penalita gravissima se le emissioni schizzano e fanno scattare il Panic Routing da 500kg
        r_panic = -1000.0 if agg_df['simulated_emission'].max() > self.global_emission_cap else 0
        
        reward = r_env + r_spa + r_mod + r_glo + r_panic
        
        done = True # Un'azione per ora campionata
...
if __name__ == "__main__":
    print("==================================================")
    print("[FAIR CITY: AI TRAINING (EPISODIC SAMPLING)]")
    print("==================================================")
    env = FairCityEnv()
    
    print("\\n[+] Spawning PPO Agent (Actor-Critic Neural Network)...")
    model = PPO("MlpPolicy", env, verbose=0, learning_rate=0.003, n_steps=128, batch_size=32)
    
    print("[+] Addestramento: l'AI affrontera 1,000 ORE DI TRAFFICO CASUALI (Fast Demo)...")
    # Nota: 1,000 steps significa vivere 1,000 ore storiche diverse scelte a caso
    model.learn(total_timesteps=1000)
    
    print("\\n--- TRAINING COMPLETATO ---")
    model.save("fair_city_rl_model")
    
    print("\\n==================================================")
    print("TEST: IL CERVELLO DINAMICO DELL'AI SUL MONDO REALE")
    print("==================================================")
    
    # 1: Rush Hour (Ora di punta)
    rush_hour_df = env.full_df[env.full_df['rush_hour'] == 1]
    ts_rush = rush_hour_df['timestamp'].iloc[0]
    
    # 2: Notte Fondamentalmente vuota
    night_df = env.full_df[(env.full_df['hour'] == 3) & (env.full_df['is_weekend'] == 1)]
    ts_night = night_df['timestamp'].iloc[0]
    
    scenarios = [
        ("LUNEDI ORE 08:00 (Rush Hour estrema - Scuole Aperte)", ts_rush),
        ("DOMENICA ORE 03:00 (Strade vuote - Nessun Pericolo)", ts_night)
    ]
    ...
    for name, ts in scenarios:
        ...
        print(f"\\n[{name}]")
        print("L'AI osserva i sensori della citta e decide autonomamente questi pesi:")
        print(f"    - Env Justice (Scuole): {action[0]*100:.1f}%")
        print(f"    - Modal Equality (Bus): {action[1]*100:.1f}%")
        print(f"    - Spatial Equity (Code): {action[2]*100:.1f}%")
